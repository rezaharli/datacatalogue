...
SELECT DISTINCT
    ...
    WHERE ('ALL' = '?' OR TP.BANK_ID = '?') AND TC.NAME IN ('USFRR' , 'LC-FCC | Global Name Screening' , 'Credit Risk | Risk Reporting | Credit Risk Reporting & MI')
    ORDER BY TC.NAME
...
SELECT DISTINCT
    ...
    FROM
        ...
        INNER JOIN TBL_MD_TABLE TAB ON TAB.ID = COL.TABLE_ID
        INNER JOIN TBL_MD_RESOURCE RES ON RES.ID = TAB.RESOURCE_ID
        INNER JOIN TBL_SYSTEM SYS ON SYS.ID = RES.SYSTEM_ID AND SYS.SYSTEM_NAME = 'USFRR'
