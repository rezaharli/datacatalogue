...
SELECT * 
	FROM (
		SELECT * 
			FROM (
				SELECT res.*, 
						COUNT(DISTINCT list_of_cde) OVER ()                 as COUNT_list_of_cde,
                        COUNT(DISTINCT imm_prec_system_name) OVER ()        as COUNT_imm_prec_system_name,
                        COUNT(DISTINCT Imm_Prec_System_SLA) OVER ()         as COUNT_Imm_Prec_System_SLA,
                        COUNT(DISTINCT Imm_Prec_System_OLA) OVER ()         as COUNT_Imm_Prec_System_OLA,
                        COUNT(DISTINCT imm_succ_system_name) OVER ()        as COUNT_imm_succ_system_name,
                        COUNT(DISTINCT Imm_Succ_System_SLA) OVER ()         as COUNT_Imm_Succ_System_SLA,
                        COUNT(DISTINCT Imm_Succ_System_OLA) OVER ()         as COUNT_Imm_Succ_System_OLA,
                        COUNT(DISTINCT list_downstream_process) OVER ()     as COUNT_list_downstream_process,
                        COUNT(DISTINCT downstream_process_owner) OVER ()    as COUNT_downstream_process_owner
                    FROM (
                        SELECT DISTINCT
                                tmt.id,
                                ts.id		                        as tsid,
