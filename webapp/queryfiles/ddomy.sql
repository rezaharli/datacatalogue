...
SELECT DISTINCT 
    ...
    FROM
        ...
        LEFT JOIN TBL_MD_TABLE TMT_GS ON TMT_GS.RESOURCE_ID = TMR_GS.ID
        LEFT JOIN TBL_MD_COLUMN TMC_GS ON TMC_GS.TABLE_ID = TMT_GS.ID AND TMC_GS.BUSINESS_TERM_ID = TBT.ID
    WHERE
        tsc.id = '?'
