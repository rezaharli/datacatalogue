<style>
  @import '../assets/styles/login.css';
</style>

<style scoped>
div#app div#login div#form {
    width: 600px;
}
</style>


<template>
  <v-content>
    <v-container fluid fill-height back>
        <div id="login">
            <div id="form">
                <b-form>
                    <b-form-group id="input-plaintext"
                            label=" "
                            label-for="plainText">
                        <b-form-input id="plainText"
                            type="text"
                            v-model="store.plainText"
                            placeholder="Enter Plain Text"></b-form-input>
                    </b-form-group>

                    <b-button size="xs" :disabled="store.isLoading" @click="encrypt">Encrypt</b-button>
                    <b-button style="float: right" size="xs" :disabled="store.isLoading" @click="decrypt">Decrypt</b-button>
                    <img v-show="store.isLoading" src="data:image/gif;base64," />

                    <b-form-group id="input-encrypted"
                            label=" "
                            label-for="encrypted">
                        <b-form-input id="encrypted"
                            type="text"
                            v-model="store.encrypted"
                            placeholder="Enter Encrypted Text"></b-form-input>
                    </b-form-group>
                </b-form>
            </div>
        </div>
    </v-container>
  </v-content>
</template>

<script>
import jQuery from 'jquery'
import { mapState, mapActions } from 'vuex'

export default {
    data() {
        return {
            submitted: false
        };
    },
    computed: {
        store () { return this.$store.state.crypto.all },
    },
    methods: {
        encrypt () {
            this.$store.dispatch(`crypto/encrypt`)
        },
        decrypt () {
            this.$store.dispatch(`crypto/decrypt`)
        },
    }
}
</script>