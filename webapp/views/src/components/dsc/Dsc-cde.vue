                ...
                <v-data-table
                  ...
                  <template slot="items" slot-scope="props">
                    <tr :class="{even: props.index % 2, odd: !(props.index % 2)}">
                      ...
                      <td v-bind:style="{ width: store.left.colWidth['DESCRIPTION'] + 'px' }" class="text-description"><tablecell :fulltext="props.item.DESCRIPTION" :isklik="true"></tablecell></td>
                      <td v-bind:style="{ width: store.left.colWidth['TABLE_NAME'] + 'px' }" class="text-uppercase"><b-link @click="props.expanded = !props.expanded"><tablecell :fulltext="props.item.TABLE_NAME" :isklik="false"></tablecell></b-link></td>
                      <td v-bind:style="{ width: store.left.colWidth['COLUMN_NAME'] + 'px' }" class="text-uppercase"><tablecell :fulltext="props.item.COLUMN_NAME" :isklik="true"></tablecell></td>
                      <td v-bind:style="{ width: store.left.colWidth['DSP_NAME'] + 'px' }" class="text-uppercase"><tablecell :fulltext="props.item.DSP_NAME" :isklik="true"></tablecell></td>
                      <td v-bind:style="{ width: store.left.colWidth['PROCESS_OWNER'] + 'px' }" class="text-capitalize"><tablecell :fulltext="props.item.PROCESS_OWNER" :isklik="true"></tablecell></td>
                    </tr>
                  </template>
                      ...
                      <template slot="expand" slot-scope="props">
                        ...
                        >
                          <template slot="items" slot-scope="props">
                            ...
                            <td class="text-uppercase" v-bind:style="{ width: store.left.colWidth['DSP_NAME'] + 'px' }"><tablecell :fulltext="(_.uniq(_.map(props.item.DspsVal, 'DSP_NAME')).filter(Boolean).join(', '))" :isklik="true"></tablecell></td>
                            <td v-bind:style="{ width: store.left.colWidth['PROCESS_OWNER'] + 'px' }"><tablecell :fulltext="(_.uniq(_.map(props.item.DspsVal, 'PROCESS_OWNER')).filter(Boolean).join(', '))" :isklik="true"></tablecell></td>
                          </template>
                        </v-data-table>
                      </template>
                    </v-data-table>
