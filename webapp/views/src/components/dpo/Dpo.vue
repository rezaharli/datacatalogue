        ...
        <b-container fluid>
            <b-row style="margin-top: 10px; margin-bottom: 20px;">
                <b-col>
                    ...
                    <page-export class="float-right" storeName="dpoall" :leftTableCols="myStore.leftHeaders" :rightTableCols="myStore.rightHeaders"/>
                </b-col>
            </b-row>
        </b-container>
...
import dpoMy from './Dpo-my.vue';
import dpoAll from './Dpo-all.vue';

export default {
    components: { PageHeader, pageExport, dpoMy, dpoAll },
    data () {
        return {}
    },
    computed: {
        store () { return this.$store.state.dpo.all },
        myStore () { return this.$store.state.dpomy.all },
        allStore () { return this.$store.state.dpoall.all }
    },
    mounted() {
        this.resetFilter();
    },
    methods: {
        getMyLeftTable () { return this.$store.dispatch(`dpomy/getLeftTable`) },
        getMyRightTable () { return this.$store.dispatch(`dpomy/getRightTable`) },
        getAllLeftTable () { return this.$store.dispatch(`dpoall/getLeftTable`) },
        getAllRightTable () { return this.$store.dispatch(`dpoall/getRightTable`) },
        resetFilter (e) {
